-- Include approved school requests in student signup school dropdown.
...
AS $$
  ...
  FROM (
    SELECT trim(p.school) AS school
    ...
    SELECT trim(srr.requested_school) AS school
    FROM public.school_registration_requests srr
    WHERE srr.status = 'approved'
      AND trim(COALESCE(srr.requested_school, '')) <> ''
  ) schools
  ORDER BY school;
$$;
