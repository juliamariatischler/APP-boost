...
public class HealthPlugin extends CordovaPlugin {
    ...
    private void checkAuthorization(final JSONArray args, boolean request) {
        try {
            ...
            if (request && !permissionsToRequest.isEmpty()) {
                ...
                final Set<String> toRequest = permissionsToRequest;
                cordova.getActivity().runOnUiThread(() -> {
                    try {
                        permissionsLauncher.launch(toRequest);
                    } catch (Exception ex) {
                        callbackContext.error("Failed to launch permissions: " + ex.getMessage());
                    }
                });
            } else {
                callbackContext.sendPluginResult(new PluginResult(PluginResult.Status.OK, true));
    ...
    private void query(final JSONArray args) {
        ...
        try {
            ...
            // done:
            Log.d(TAG, "Raw " + datatype + " response count=" + resultset.length() + " data=" + resultset.toString());
    ...
    private void queryAggregated(final JSONArray args) {
        try {
            ...
            if (hasbucket) {
                ...
                Log.d(TAG, "Aggregated " + datatype + " response: " + retObject.toString());
